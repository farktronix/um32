...
D        j = I
DV       DIM pass AS STRING
DX       pass = words(i) + j
DXV      IF CHECKPASS(username, pass) THEN GOTO DXLV
DXX      j = j + I
DXXV     IF j < C THEN GOTO DX 
DXXX     i = i + I
DXXXV    IF i > pwdcount THEN GOTO DLX
DXL      GOTO DX 
DXLV     PRINT "found match!! for user " + username + CHR(X) 
DL       PRINT "password: " + pass + CHR(X) 
DLV      END 
DLX      PRINT "no matches found for user " + username + CHR(X) 
DLXV     END
